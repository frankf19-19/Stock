...
def main():
    ...
    for idx, s in enumerate(stocks):
        ...
        if not sid: continue
        path = f"m/{sid}.json"
        # r690 修正:CI 上 checkout 的 mtime 永遠是「剛剛」,原條件把全庫永久略過(2787/2795)。
        # 改以「內容月份」判斷:最後月份=當月且未過月中才略過;跨月首跑/月中跑=全量刷新,
        # 順便以完整重抓把上月凍結的月底值蓋成正確終值。
        cur = time.strftime("%Y-%m")
        mid = int(time.strftime("%d")) >= 14
        if os.path.exists(path):
            try:
                j = json.load(open(path, encoding="utf-8"))
                if j.get("d") and j["d"][-1] == cur and not mid:
                    skip += 1; continue
            except Exception:
                pass
        if mkt == "TW":
