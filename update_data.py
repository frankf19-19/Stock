...
K_DIR = "k"
KEEP_BARS = 780        # r646:日K保留 780 個交易日(約三年)——週K「全部」才有縱深;回補引擎逐日磨深

# ── 籌碼歷史(v5 新增):法人逐日 / 大戶逐週 / 營收逐月,分片存 c/*.json ──
C_DIR = "c"            # 籌碼歷史分片資料夾
CHIP_DAYS = 390        # r646:法人買賣超保留 390 個交易日(約一年半,逐日累積加深)
BIG_WEEKS = 52         # 400張大戶保留 52 週(約一年)
REV_MONTHS = 40        # 月營收保留 40 個月(三年以上年增趨勢)
CRED_DAYS = 130        # 融資/融券/借券賣出餘額 逐日保留 130 個交易日
CHIP_BACKFILL = 300    # 單次執行最多回補幾個交易日的法人資料(一年份一次跑滿,約 10~12 分鐘)
...
def fetch_conf_calendar(stocks):
    ...
    def roc_date(txt):
        t = str(txt).strip()
        m2 = _r.search(r"(\d{2,4})[/\-.](\d{1,2})[/\-.](\d{1,2})", t)
        if not m2:
            m2 = _r.match(r"^(\d{3})(\d{2})(\d{2})$", t) or _r.match(r"^(\d{4})(\d{2})(\d{2})$", t)
        if not m2:
            return None
        try:
            y = int(m2.group(1)); y = y + 1911 if y < 1900 else y
            return dt.date(y, int(m2.group(2)), int(m2.group(3)))
        except Exception:
            return None
    ...
    try:
        ...
        pass
    def _post(url, data):
        for u in ([url] + ([_pu + "/?url=" + _up.quote(url, safe="")] if _pu else [])):
            try:
                r = requests.post(u, data=data, headers=UA, timeout=60)
